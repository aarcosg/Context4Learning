...
public class MessagingEndpoint {
    ...
    public void sendMessages(@Named("message") String message) throws IOException {
        ...
        for (Device device : devices) {
            Result result = sender.send(msg, device.getInstanceId(), 5);
            if (result.getMessageId() != null) {
                ...
                if (canonicalGCMId != null) {
                    ...
                    log.info("Registration Id changed for " + device.getInstanceId() + " updating to " + canonicalGCMId);
                    device.setInstanceId(canonicalGCMId);
                    ofy().save().entity(device).now();
                }
            } else {
                String error = result.getErrorCodeName();
                if (error.equals(Constants.ERROR_NOT_REGISTERED)) {
                    log.warning("Registration Id " + device.getInstanceId() + " no longer registered with GCM, removing from datastore");
    ...
    public void sendMessage(@Named("message") String message, @Named("deviceId") Long deviceId) throws IOException {
        ...
        if (result.getMessageId() != null) {
            log.info("Message sent to " + device.getId());
            new NotificationCounterEndpoint().insert(device.getUser().getId());
            String canonicalGCMId = result.getCanonicalRegistrationId();
            if (canonicalGCMId != null) {
                // if the regId changed, we have to update the datastore
                log.info("Registration Id changed for " + device.getInstanceId() + " updating to " + canonicalGCMId);
    ...
    public void sendNotification(Notification notification){
        Device device = notification.getContext().getDevice();
        log.info("Prepare notification. Trying to send message to device: " + device.getId());
        Sender sender = new Sender(API_KEY);
        Message msg = new Message.Builder()
                .addData("notification_id", notification.getId().toString())
                .addData("message", notification.getMessage())
                .build();

        Result result = null;
        try {
            result = sender.send(msg, device.getInstanceId(), 5);
        } catch (IOException e) {
